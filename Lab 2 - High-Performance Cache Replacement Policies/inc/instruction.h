...
// instruction format
#define ROB_SIZE 224
#define REG_OCCUPANCY 97
#define LQ_SIZE 72
#define SQ_SIZE 56
#define NUM_INSTR_DESTINATIONS_SPARC 4
#define NUM_INSTR_DESTINATIONS 2
#define NUM_INSTR_SOURCES 4
#define CACHELINEWORDS 8
#define OPSIZE 16

// Total number of histories and access counters
#define HIST 2
#define ACNT 1024
...
public:
    ...
    // instruction size and its opcode bytes
    uint8_t ip_size;
    ...
    uint64_t destination_data[NUM_INSTR_DESTINATIONS][CACHELINEWORDS]; // destination data
    uint64_t source_memory[NUM_INSTR_SOURCES]; // input memory
    uint64_t source_data[NUM_INSTR_SOURCES][CACHELINEWORDS]; // source data

    uint64_t ip_data[CACHELINEWORDS]; //ip cacheline
...
public:
    ...
    uint64_t destination_data[NUM_INSTR_DESTINATIONS_SPARC][CACHELINEWORDS]; // destination data
    uint64_t source_memory[NUM_INSTR_SOURCES]; // input memory
    uint64_t source_data[NUM_INSTR_SOURCES][CACHELINEWORDS]; // source data
...
public:
   ...
   //Specific Information about the instr.
    uint8_t ip_size;
    uint8_t opcode[OPSIZE];
    uint64_t ip_data[CACHELINEWORDS];
    ...
    void print_instr()
    {
        ...
        for(uint32_t i=0; i<NUM_INSTR_SOURCES; i++)
        {
            cout << hex << "SAddr 0x" << (uint32_t)source_memory[i] << " Data 0x";
            for (uint32_t j=0; j<CACHELINEWORDS; j++) {
        ...
        for(uint32_t i=0; i<NUM_INSTR_DESTINATIONS; i++)
        {
            cout << hex << "DAddr 0x" << (uint32_t)destination_memory[i] << " Data 0x";
            for (uint32_t j=0; j<CACHELINEWORDS; j++) {
                cout << destination_data[i][j];
            }
            cout << dec << " ";
        }
        cout << endl;
