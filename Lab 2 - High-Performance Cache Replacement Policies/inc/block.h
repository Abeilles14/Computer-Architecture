...
public:
    ...
    //Data Values and Compressed Size (in Bits)
    uint64_t data_value[CACHELINEWORDS];
    uint16_t comp_size;
    
    //How hot the line is
    uint32_t reusestamp;
    uint16_t distance[HIST];
    uint8_t currentuse;
    ...
    BLOCK() {
        ...
        for (uint16_t i=0; i<HIST; i++) {
            distance[i] = 0;
        }
        
        //Data Values and Compressed Values
