...
void CACHE::operate()
{
    ...
    //Add an additional function that promotes hot lines to highest position
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
    ...
    for (uint64_t i=0; i<CACHELINEWORDS; i++) {
        block[set][way].data_value[i] = packet->data_value[i];
    }
    
    // ====== Compression Algorithm Here =========
    // Compress only the LLC and only data cachelines as we do not have the content in the instruction cachelines
    if(cache_type == IS_LLC){
        block[set][way].comp_size = compress(packet, BDI_FPC_OPT);
    }
    else{
        block[set][way].comp_size = 64;
    }
    // ===========================================
...
// ====== Compression Part Starts =========
uint16_t CACHE::compress(PACKET *packet, int technique)
{
    //64 Byte Blocks
    uint16_t _blockSize = 64;
    ...
    switch (technique)
    {
        case NOCOMP:
            ...
            break;
        case BDI:
            return BDICompress(packet, _blockSize);
            break;
        case FPC:
            //std::cout << "block-size: " << _blockSize << "\n";
            return FPCCompress(packet, _blockSize/4);
            break;
        case BDI_FPC_OPT:
        {
            unsigned BDISize = BDICompress(packet, _blockSize);
            unsigned FPCSize = FPCCompress(packet, _blockSize/4);
            if(BDISize <= FPCSize)
                return BDISize;
            else
                return FPCSize;
...
unsigned CACHE::FPCCompress(PACKET *packet, unsigned size){
    uint64_t * values;
    values = new uint64_t [size];
    
    for (unsigned int i = 0; i < size; i++) {
        values[i] = packet->data_value[i/2];
        if(i%2 == 0){
            values[i] = values[i] & 0x00000000FFFFFFFF;
        }
        else{
            values[i] = values[i] & 0xFFFFFFFF00000000;
...
unsigned CACHE::BDICompress(PACKET *packet, unsigned _blockSize){
    uint64_t * values1;
    uint64_t * values2;
    uint64_t * values3;
    values1 = new uint64_t [_blockSize/8];
    values2 = new uint64_t [_blockSize/4];
    values3 = new uint64_t [_blockSize/2];
    
    for (unsigned int i = 0; i < _blockSize/8; i++) {
        values1[i] = packet->data_value[i];
    }
    
    for (unsigned int i = 0; i < _blockSize/4; i++) {
        values2[i] = packet->data_value[i/2];
        if(i%2 == 0){
            values2[i] = values2[i] & 0x00000000FFFFFFFF;
        }
        else{
            values2[i] = values2[i] & 0xFFFFFFFF00000000;
            values2[i] = values2[i] >> 32;
        }
    }
    
    for (unsigned int i = 0; i < _blockSize/2; i++) {
        values3[i] = packet->data_value[i/4];
        if(i%4 == 0){
            values3[i] = values3[i] & 0x000000000000FFFF;
        }
        else if(i%4 == 1){
            values3[i] = values3[i] & 0x00000000FFFF0000;
            values3[i] = values3[i] >> 16;
        }
        else if(i%4 == 2){
            values3[i] = values3[i] & 0x0000FFFF00000000;
            values3[i] = values3[i] >> 32;
        }
        else{
            values3[i] = values3[i] & 0xFFFF000000000000;
            values3[i] = values3[i] >> 48;
        }
    }
    
    unsigned bestCSize = _blockSize;
    unsigned currCSize = _blockSize;
    if( isZeroPackable( values1, _blockSize / 8))
        bestCSize = 1;
    if( isSameValuePackable( values1, _blockSize / 8))
        currCSize = 8;
    bestCSize = bestCSize > currCSize ? currCSize: bestCSize;
    currCSize = multBaseCompression( values1, _blockSize / 8, 1, 8);
    bestCSize = bestCSize > currCSize ? currCSize: bestCSize;
    currCSize = multBaseCompression( values1, _blockSize / 8, 2, 8);
    bestCSize = bestCSize > currCSize ? currCSize: bestCSize;
    currCSize =  multBaseCompression( values1, _blockSize / 8, 4, 8);
    bestCSize = bestCSize > currCSize ? currCSize: bestCSize;
    
    if( isSameValuePackable( values2, _blockSize / 4))
        currCSize = 4;
    bestCSize = bestCSize > currCSize ? currCSize: bestCSize;
    currCSize = multBaseCompression( values2, _blockSize / 4, 1, 4);
    bestCSize = bestCSize > currCSize ? currCSize: bestCSize;
    currCSize = multBaseCompression( values2, _blockSize / 4, 2, 4);
    bestCSize = bestCSize > currCSize ? currCSize: bestCSize;
    
    currCSize = multBaseCompression( values3, _blockSize / 2, 1, 2);
    bestCSize = bestCSize > currCSize ? currCSize: bestCSize;
...
unsigned CACHE::multBaseCompression(uint64_t * values, unsigned size, unsigned blimit, unsigned bsize){
...
unsigned CACHE::doubleExponentCompression(uint64_t * values, unsigned size, unsigned blimit, unsigned bsize){
    unsigned long long limit = 0;
    //define the appropriate size for the mask
    switch(blimit){
        case 1:
...
int CACHE::isSameValuePackable (uint64_t * values, unsigned size){
    int notSame = 0;
    for (unsigned int i = 0; i < size; i++) {
...
int CACHE::isZeroPackable (uint64_t * values, unsigned size){
    int nonZero = 0;
    for (unsigned int i = 0; i < size; i++) {
        if( values[i] != 0){
            nonZero = 1;
...
uint64_t CACHE::my_llabs(int64_t x)
{
    uint64_t t = x >> 63;
    return (x ^ t) - t;
}

uint32_t CACHE::my_abs (int32_t x )
{
    uint32_t t = x >> 31;
    return (x ^ t) - t;
}

// ====== Compression Part Ends =========
...
int CACHE::add_rq(PACKET *packet)
{
    ...
    if (wq_index != -1) {
        ...
        if (packet->fill_level < fill_level) {
            ...
            for (uint64_t i=0; i<CACHELINEWORDS; i++) {
                packet->data_value[i] = WQ.entry[wq_index].data_value[i];
            }
...
int CACHE::add_wq(PACKET *packet)
{
    ...
    if (index != -1) {
        for (uint64_t i=0; i<CACHELINEWORDS; i++) {
            WQ.entry[index].data_value[i] = packet->data_value[i];
        }
        WQ.MERGED++;
        WQ.ACCESS++;
...
int CACHE::add_pq(PACKET *packet)
{
    ...
    if (index != -1) {
        ...
        for (uint64_t i=0; i<CACHELINEWORDS; i++) {
            PQ.entry[index].data_value[i] = packet->data_value[i];
        }
...
void CACHE::llc_profile_final_stats()
{
    cout << endl << " ------------ " << endl;
}
