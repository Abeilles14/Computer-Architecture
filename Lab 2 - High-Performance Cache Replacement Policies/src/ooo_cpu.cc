    ...
    while (continue_reading) {
        ...
        if (knob_cloudsuite) {
            if (!fread(&current_cloudsuite_instr, instr_size, 1, trace_file)) {
                ...
                //-- ip data and opcodes --
                arch_instr.ip_size = current_cloudsuite_instr.ip_size;
                for (uint32_t i=0; i<OPSIZE; i++) {
                    arch_instr.opcode[i] = current_cloudsuite_instr.opcode[i];
                }
                for (uint32_t i=0; i<CACHELINEWORDS; i++) {
                    arch_instr.ip_data[i] = current_cloudsuite_instr.ip_data[i];
                }
                //-------------------------
                ...
                for (uint32_t i=0; i<MAX_INSTR_DESTINATIONS; i++) {
                    ...
                    for (uint32_t j=0; j<CACHELINEWORDS; j++) {
                        arch_instr.destination_data[i][j] = current_cloudsuite_instr.destination_data[i][j];
                    }
                ...
                for (int i=0; i<NUM_INSTR_SOURCES; i++) {
                    ...
                    for (uint32_t j=0; j<CACHELINEWORDS; j++) {
                        arch_instr.source_data[i][j] = current_cloudsuite_instr.source_data[i][j];
                    }
        ...
        else
        {
            ...
            { // successfully read the trace
                ...
                //-- ip data and opcodes --
                arch_instr.ip_size = current_instr.ip_size;
                for (uint32_t i=0; i<OPSIZE; i++) {
                    arch_instr.opcode[i] = current_instr.opcode[i];
                }
                for (uint32_t i=0; i<CACHELINEWORDS; i++) {
                    arch_instr.ip_data[i] = current_instr.ip_data[i];
                }
                //-------------------------
                ...
                for (uint32_t i=0; i<MAX_INSTR_DESTINATIONS; i++) {
                    ...
                    for (uint32_t j=0; j<CACHELINEWORDS; j++) {
                        arch_instr.destination_data[i][j] = current_instr.destination_data[i][j];
                    }
                ...
                for (int i=0; i<NUM_INSTR_SOURCES; i++) {
                    ...
                    for (uint32_t j=0; j<CACHELINEWORDS; j++) {
                        arch_instr.source_data[i][j] = current_instr.source_data[i][j];
                    }
...
void O3_CPU::fetch_instruction()
{
    ...
    for(uint32_t i=0; i<IFETCH_BUFFER.SIZE; i++)
    {
        ...
        if(IFETCH_BUFFER.entry[index].translated == 0)
        {
            ...
            //Instruction Cache-Line Content Values are Unknown.
            for(uint32_t k=0; k<CACHELINEWORDS; k++)
            {
                trace_packet.data_value[k] = IFETCH_BUFFER.entry[index].ip_data[k];
            }
        ...
        if((IFETCH_BUFFER.entry[index].translated == COMPLETED) && (IFETCH_BUFFER.entry[index].fetched == 0))
        {
            ...
            for(uint32_t k=0; k<CACHELINEWORDS; k++)
            {
                fetch_packet.data_value[k] = IFETCH_BUFFER.entry[index].ip_data[k];
            }
...
void O3_CPU::decode_and_dispatch()
{
    ...
    for(uint32_t i=0; i<DECODE_BUFFER.SIZE; i++)
    {
        ...
        //Check if there is space in ROB
        if((ROB.reg_occupancy > REG_OCCUPANCY) && (DECODE_BUFFER.entry[DECODE_BUFFER.head].is_memory == 0))
        {
            break;
        }
           ...
           ((DECODE_BUFFER.entry[DECODE_BUFFER.head].event_cycle != 0) && (DECODE_BUFFER.entry[DECODE_BUFFER.head].event_cycle < current_core_cycle[cpu]) && (ROB.occupancy < ROB.SIZE)))
        {
            ...
            if(DECODE_BUFFER.entry[DECODE_BUFFER.head].is_memory == 0)
            {
                assert(ROB.reg_occupancy <= REG_OCCUPANCY);
                ROB.reg_occupancy++;
            }
...
void O3_CPU::add_load_queue(uint32_t rob_index, uint32_t data_index)
{
    ...
    for (uint64_t i=0; i<CACHELINEWORDS; i++) {
        LQ.entry[lq_index].data_value[i] = ROB.entry[rob_index].source_data[data_index][i];
    }
    LQ.occupancy++;
    ...
    if (forwarding_index != SQ.SIZE) { // we have a store-to-load forwarding
        ...
        if ((SQ.entry[forwarding_index].fetched == COMPLETED) && (SQ.entry[forwarding_index].event_cycle <= current_core_cycle[cpu])) {
            ...
            for (uint64_t k=0; k<CACHELINEWORDS; k++) {
                LQ.entry[lq_index].data_value[k] = SQ.entry[forwarding_index].data_value[k];
            }
...
void O3_CPU::add_store_queue(uint32_t rob_index, uint32_t data_index)
{
    ...
    for (uint64_t i=0; i<CACHELINEWORDS; i++) {
        SQ.entry[sq_index].data_value[i] = ROB.entry[rob_index].destination_data[data_index][i];
    }
    ...
    while (store_issued < SQ_WIDTH) {
        if (RTS0[RTS0_head] < SQ_SIZE) {
            ...
            if (SQ.entry[sq_index].event_cycle <= current_core_cycle[cpu]) {
                ...
                for(uint32_t k=0; k<CACHELINEWORDS; k++)
                {
                    data_packet.data_value[k] = SQ.entry[sq_index].data_value[k];
                }
                DP (if (warmup_complete[cpu]) {
    ...
    while (load_issued < LQ_WIDTH) {
        if (RTL0[RTL0_head] < LQ_SIZE) {
            ...
            if (LQ.entry[lq_index].event_cycle <= current_core_cycle[cpu]) {
                ...
                for(uint32_t k=0; k<CACHELINEWORDS; k++)
                {
                    data_packet.data_value[k] = LQ.entry[lq_index].data_value[k];
                }
                DP (if (warmup_complete[cpu]) {
            ...
            for (uint32_t j=0; j<NUM_INSTR_SOURCES; j++) { // which one is dependent?
                if (ROB.entry[dependent].source_memory[j] && ROB.entry[dependent].source_added[j]) {
                    if (ROB.entry[dependent].source_memory[j] == SQ.entry[sq_index].virtual_address) { // this is required since a single instruction can issue multiple loads
                        ...
                        for(uint32_t k=0; k<CACHELINEWORDS; k++)
                        {
                            LQ.entry[lq_index].data_value[k] = SQ.entry[sq_index].data_value[k];
                        }
