...
#define NON_PIPELINED_BANDWIDTH 1

void O3_CPU::operate()
{
  // minimum b/w fetch width and ifetch buffer available space
  instrs_to_read_this_cycle = (IFETCH_BUFFER.occupancy() != 1);
  // instrs_to_read_this_cycle = std::min(NON_PIPELINED_BANDWIDTH, IFETCH_BUFFER.size() - IFETCH_BUFFER.occupancy());
...
{
  unsigned available_fetch_bandwidth = NON_PIPELINED_BANDWIDTH;
...
{
  std::size_t available_decode_bandwidth = NON_PIPELINED_BANDWIDTH;
...
void O3_CPU::dispatch_instruction()
{
  ...
  std::size_t available_dispatch_bandwidth = NON_PIPELINED_BANDWIDTH;
...
void O3_CPU::handle_memory_return()
{
  ...
  std::size_t available_fetch_bandwidth = NON_PIPELINED_BANDWIDTH;
  ...
  available_fetch_bandwidth = NON_PIPELINED_BANDWIDTH;
...
{
  unsigned retire_bandwidth = NON_PIPELINED_BANDWIDTH;
